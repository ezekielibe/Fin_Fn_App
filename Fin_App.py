    ...
    if choice == 'Homepage':
        ...
        if option =='Present_Value':
            ...
            def preferred_stock_valuation(dividend, required_rate_of_return):
                ...
                psv_value = str("### **Preferred Stock Valuation is given as** "+str("%.2f" %psv))
                st.write(psv_value)
                data_4 = {'Computed preferred stock value is ': psv_value}
                result = pd.DataFrame(data_4, index=[0])
                return result
            ...
            def user_input_features():
                interest_rate = st.sidebar.slider('Kindly enter interest_rate', 0.01,1.00)
                tax_rate = st.sidebar.slider('Kindly enter tax_rate', 0.01, 1.00)
